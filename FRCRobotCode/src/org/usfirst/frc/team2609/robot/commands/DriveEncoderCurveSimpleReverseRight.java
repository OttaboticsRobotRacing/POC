...
public class DriveEncoderCurveSimpleReverseRight extends Command {
	SimPID drivePIDLeft;
	...
	public DriveEncoderCurveSimpleReverseRight(double driveTarget, double drivePower, double gyroMax, double driveHeading1, double driveHeading2, double driveHeading3, double driveHeading4, double curvePoint1, double curvePoint2, double curvePoint3) {
    ...
    protected void initialize(){
        ...
        gyroP = (double)SmartDashboard.getNumber("Curve P: ",0);
        gyroI = (double)SmartDashboard.getNumber("Curve I: ",0);
        gyroD = (double)SmartDashboard.getNumber("Curve D: ",0);
    ...
    protected void execute() {
    	...
    	if (Math.abs(RobotMap.driveTalonRight1.getPosition())<Math.abs(curvePoint1)){
    		System.out.println("curvepoint1---------------------------------------");
            this.steeringPID.setDesiredValue(driveHeading1);
    	}
    	else if (Math.abs(RobotMap.driveTalonRight1.getPosition())<Math.abs(curvePoint2)){
    		System.out.println("curvepoint2---------------------------------------");
            this.steeringPID.setDesiredValue(driveHeading2);
    	}
    	else if (Math.abs(RobotMap.driveTalonRight1.getPosition())<Math.abs(curvePoint3)){
    	...
    	Robot.drivetrain.driveStraightReverseRight(RobotMap.driveTalonLeft1.getPosition(), RobotMap.driveTalonRight1.getPosition(), reverseAngle, drivePIDLeft, drivePIDRight, steeringPID);	
    }

    protected boolean isFinished() {
    	return drivePIDRight.isDone();//&& drivePIDRight.isDone(); //Should wait for both PID to finish
    }
